...
void WaterTile::Draw1(CrossingRoadGame& game) {
...
void WaterTile::Draw2(CrossingRoadGame& game) {
	//Vi du ve 1 o 16x16 mau den
	/*game.Fill(x, y,x + 16, y+ 16 ,L' ', FG_BLACK);*/
	//LANE NUOC
	game.Fill(x, y, x + width - 1, y + height - 1, L' ', COLOUR::BG_BLUE);
...
void WaterTile::Draw(CrossingRoadGame& game) {
	if (spriteIndex) {
		Draw1(game);
	}
	else {
...
void WaterTile::Update(CrossingRoadGame& game, float fElapsedTime) {
	time += fElapsedTime;
	if (time > 0.3f) {
